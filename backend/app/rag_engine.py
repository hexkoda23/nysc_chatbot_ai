...
def get_agent(target_lang: str = "en"):
    ...
    system_prompt = (
        ...
        f"{lang_instruction}\n\n"
        "STRICT RULES:\n"
        "- Answer ONLY the Current Question. Do NOT summarize or repeat the previous conversation history.\n"
        f"- Write in plain {lang_name}, no markdown asterisks, no ALL CAPS\n"
        "- Give SPECIFIC information: exact amounts, exact URLs, exact steps\n"
        "- If local docs and web data conflict, prefer the most recent information\n"
...
def _fast_rag_response(message: str, conversation_context: str = "", target_lang: str = "en") -> Dict[str, Any]:
    ...
    SYSTEM_RULES = (
        "You are NYSC AI — a knowledgeable assistant that answers ONLY questions about the Nigerian National Youth Service Corps (NYSC) programme.\n\n"
        "STRICT RULES:\n"
        "1. Answer ONLY the User question at the bottom. Do NOT summarize or repeat the previous conversation.\n"
        "2. Answer ONLY using the provided context. If the context does not contain the answer, say exactly: 'I don't have specific information on that in my documents. Please visit `https://www.nysc.gov.ng`  or call the NYSC helpline.'\n"
        "3. If the retrieved context is clearly irrelevant to the question (e.g., procurement text for an allowance question), IGNORE it and use rule 1.\n"
        f"4. Write in plain conversational {lang_name}. No markdown, no asterisks, no ALL CAPS.\n"
        "5. Structure your answer: one clear opening sentence, then numbered steps or details if needed, then an official source line at the end.\n"
        "6. Never exceed 150 words unless the question genuinely requires detail.\n"
        f"7. After your answer, on a new line write '{follow_up_txt}' followed by 2 short relevant follow-up questions numbered 1. and 2.\n"
        f"   {lang_instruction}\n"
        "8. Always end with:\n"
        ...
        "- NYSC monthly allowance is N77,000 per month (approved by Federal Government, September 2024, effective March 2025)\n"
        "- This replaced the previous N33,000 allowance — ALWAYS use N77,000 in any answer about NYSC allowance\n"
        "- SAED = Skill Acquisition and Entrepreneurship Development, compulsory in camp since 2012\n"
        "- Best SAED skills in 2025: Digital Marketing, Web Design, Solar Installation, Photography, Fashion\n"
        "- SAED is free during camp, post-camp training may have small fees\n"
        "- SAEDConnect platform: https://saedconnect.org\n"
        "- CDS = Community Development Service, compulsory weekly (once per week, 8am-12pm)\n"
        "- There are 16 official CDS groups — corps members are assigned by LGI based on field of study\n"
        "- Missing over 50% of CDS in a month = monthly clearance not signed = allowance blocked\n"
        "- Best CDS groups: ICT (for tech), Health (for sciences), Legal Aid (for lawyers), Sports (relaxed)\n"
        "- CDS source: https://www.nysc.gov.ng/cds.html\n"
    )
